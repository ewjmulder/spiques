package com.programyourhome.spiques.master.server;
...
public class SpiquesMasterServer {

	private static boolean shutdown = false;

    public static void startServer(final Class<?> springBootApplicationClass) {
        final String usageMessage = "Please provide the correct path to the SPiQues master property location with: -Dspiques.master.properties.location=/path/to/file";
        final String spiquesPropertyLocation = System.getProperty("spiques.master.properties.location");
        if (spiquesPropertyLocation == null) {
            System.out.println("No value provided for property 'spiques.master.properties.location'.");
    ...
    private static class ShutdownPoller extends Thread {
        ...
        @Override
        public void run() {
            while (!SpiquesMasterServer.shutdown) {
                try {
                    Thread.sleep(100);
